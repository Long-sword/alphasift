...
def _configure_tushare_client(pro: object, *, token: str) -> None:
    ...
    # ``http://api.waditu.com/dataapi`` (note the ``/dataapi`` suffix). Older
    # versions of this helper set ``_DataApi__http_url`` to a bare host
    # (``http://api.waditu.com``), which made every query POST to
    # ``/daily`` / ``/daily_basic`` and get a 404, so daily() came back empty.
    # Only override the URL when the user explicitly asked for one, and always
    # normalize it to end with ``/dataapi``.
    explicit_url = (
